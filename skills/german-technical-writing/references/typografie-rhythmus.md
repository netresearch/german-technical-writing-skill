...
**Zahlen, Datum, Uhrzeit.** Decimal comma, thousands point: *1.234,56*, *199,95 €*. Time with colon plus *Uhr*: *18:00 Uhr*. Date *23.07.2026* or ISO *2026-07-23*; with month name no leading zero (*7. März*). Never a bare *MM-DD* (*09-11*, *Snapshot von 09-02*) — it is a truncated ISO date that reads as September 11th to one reader and as nothing at all to another, and it carries no year, which is the part that matters once a ticket is a month old. Write the full ISO date, or German *11.09.* when the year is unambiguous from the surrounding text. Units keep a space: *512 MB*, *2 GB*.
...
**Jeder Mechanismus wird einmal erklärt — nicht jede Wiederholung ist Padding.** Explain a cause, a mechanism or a decision once, in full, where it matters most; do not explain it a second time. Repeat the terms and the one-clause context a later section needs to stand on its own: a *siehe oben* or a *dadurch* whose referent the reader has to search for costs more than the four repeated words save. One statement per fact still holds locally — if two adjacent sentences say the same thing in different words, keep the better one. Which repetitions are required: `cognitive-accessibility.md`.

**Satzlängen variieren.** Natural prose mixes a short verdict sentence with a longer explanatory one. A run of same-length, same-structure main clauses (Behauptung → Ausführung → Konsequenz, repeated per paragraph) is the signature AI rhythm — break it. The same goes for matched parallel verdicts across items (*„Löst X nicht." / „Verschiebt X nur."*): vary the build. Not where the parallelism carries meaning — acceptance criteria, a list of findings, several checks of one kind read clearer in the same shape, and breaking their symmetry to sound less machine-written makes them harder to compare.
...
**Fachbegriffe werden nicht selbst glossiert.** Writing *„… wächst unbegrenzt (Memory Leak)"* to a dev audience explains a term the reader knows — drop the gloss and use the term directly, or the plain description alone. This is about established terminology in front of the audience that uses it: a term outside the reader's field and a coinage invented in this project are introduced once at first use (`cognitive-accessibility.md`).
...
A quick scan after composing: any `—`? More than two `–`-Einschübe in the whole text, or dash-punchlines closing several paragraphs? English quotes? An uncoupled English compound, a needlessly hyphenated simple one (*Speicher-Limit*), a compound you minted yourself that the reader cannot look up (*Intent-Defekt*), or the same term spelled two ways? The same fact stated twice, or a *siehe oben* whose referent the reader has to search for? Three symmetric anything that is not genuinely parallel? A term glossed for people who coined it? A *„Damit ist … ausgeschlossen"*? A closing summary? A lone slang participle in a formal text? A Jira comment collapsed into one block? If yes — fix before posting.
