...
Real paired cases from past Jira comments and MR descriptions. Each pair shows the anglicism-laden original, the rewrite, and why the rewrite works.
...
## Case 3 — MR description

**Bad:**

> Dieser MR fixed den Bug wo die Exception gecatched wurde aber nicht rethrown, sodass der Error silent war. Ich habe das gehandled indem ich den Handler refactored habe.

**Problems:**

- Stacked Denglisch with no reason: *fixed*, *gecatched*, *rethrown*, *silent*, *gehandled*, *refactored*
- First person (*"Ich habe"*) in an MR description
...
- Impersonal — no *"ich"*
- The MR describes the outcome (*"der Handler ist umgestellt"*), not the author's process
...
> HMKG-2202: fixed the thing where the provider was returning null when product had no pim fields — added a try catch

**Problems:**

- Not actually a problem for HMKG where commits are English (per CLAUDE.md), but if this were a German-speaking project:
- Past tense + first-person narrative is wrong
- *fixed*, *returning*, *try catch* not translated

**Good (German project):**

> HMKG-2202: Provider-Fehler bei fehlenden PIM-Feldern abfangen

**Why:**

- Imperative/infinitive summary ("catch provider error when PIM fields are missing")
- Describes intent, not process
- Hyphenated compound nouns for clarity

**Good (HMKG project, English per convention):**

> HMKG-2202: Close PDP LogicException on pim_* extend fields

## Case 7 — Status update comment
...
## Case 8 — Deployment announcement
